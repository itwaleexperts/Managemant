...
const Bookings = () => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`https://apiyatraadda.jaspersoftwaresolutions.com/api/bookings/${id}`); 
            ...
            {bookings.length > 0 ? (
              bookings.map((b) => (
                <tr key={b._id} className="border-b hover:bg-gray-50">
                  ...
                  <td className="px-4 py-2">{b.hoyelId?.name}</td>
                  ...
                  <td className="px-4 py-2 font-semibold text-green-700">
                    ₹ {(b.roomPrice + b.cityTax).toLocaleString()}
                  </td>
